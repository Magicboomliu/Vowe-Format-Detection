...
if __name__ == "__main__":
    dct = unpickle("pickle_files/16_8pickle/mfcc14_LEFT")
    # trainig data and trainig data labels
    trainig_data = dct["MFCC"]
    trainig_data_labels =dct["labels"]
    print(len(set(trainig_data_labels)))
    trainig_data_labels = to_categorical(trainig_data_labels,num_classes=14)
    ...
    x = layers.Conv1D(32,3,1,activation='relu',padding='same')(input_tensor)
    ...
    history = model.fit(trains_d,trains_l,epochs=1000,batch_size=1024,validation_data=(val_d,val_l),
    callbacks=callback)

    score = model.evaluate(val_d,val_l)
    print(score)

    model.save("model/16_8/DNN/MFCC14_LEFT")
    ...
    plot_confusion_matrix(cm = confusion_matrix,labels_name=labels_name,title="16_8-DNN(L),acc is {}"
     .format(round(acc,2)), figname="16_8DNN_L")
