...
Waveform loadWAV(const std::string& filename) {
	...
	int cnt = -1;
	for (int i = 0; i < 200;i++) {
	auto* dataHeader = reinterpret_cast<wavChunkHeader*>(reinterpret_cast<char*>(&hdr.format.format) + hdr.format.chunkHeader.chunkSize+i);
	cnt++;
	...
	auto* dataHeader = reinterpret_cast<wavChunkHeader*>(reinterpret_cast<char*>(&hdr.format.format) + hdr.format.chunkHeader.chunkSize + cnt);
	if (!isChunkIDEqual(dataHeader->chunkID, "data"))
