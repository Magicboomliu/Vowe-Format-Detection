...
constexpr char kOutputSpecturm[] = "spectrogram_matrix";
constexpr char kInputStream[] = "mono_waveform";

mediapipe::Matrix packet_specturm;
...
// 建立图的输出
graph->ObserveOutputStream(kOutputSpecturm,
                             [&](const mediapipe::Packet& specturm_packet) {
                                packet_specturm = specturm_packet.Get<mediapipe::Matrix>();
...
LOG(INFO)<<"这个是 packet_specturm 输出cols:"<<packet_specturm.cols()<<"\n";
LOG(INFO)<<"这个是 packet_specturm 输出rows:"<<packet_specturm.rows()<<"\n";
}
