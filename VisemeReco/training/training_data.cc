...
void libsvm_formant(svm_problem& prob, vector<vector<double>> data,
                    vector<double> label) {
  // 记录一共有多少样本，以及样本的维度
...
// 得到训练的正确率
double acc_rate(svm_model* model, vector<vector<double>> data,
                vector<double> label) {
  svm_problem prob;
...
void training_svm(int type, const char* absolute_saved_path) {
  ...
  if (type == 0) {
    readData_proto(
        "experimental/zihualiu/apps/VisemeReco/training/data/mfcc5_data_proto",
        mfcc5_data);
    readlabel_proto(
        "experimental/zihualiu/apps/VisemeReco/training/data/mfcc5_label_proto",
        mfcc5_label);
  } else if (type == 1) {
    readData_proto(
        "experimental/zihualiu/apps/VisemeReco/training/data/mfcc9_data_proto",
        mfcc5_data);
    readlabel_proto(
        "experimental/zihualiu/apps/VisemeReco/training/data/mfcc9_label_proto",
        mfcc5_label);
  } else {
    cout << "type is only support '0' and '1' , '0' is 5 category, '1' is 9 "
            "category"
         << endl;
    return;
  }
  ...
  training_data = vector<vector<double>>(
      mfcc5_data.begin(), mfcc5_data.begin() + prob_training_nums);
  training_label = vector<double>(mfcc5_label.begin(),
                                  mfcc5_label.begin() + prob_training_nums);

  vector<vector<double>> testing_data;
  vector<double> testing_label;
  testing_data = vector<vector<double>>(
      mfcc5_data.begin() + prob_training_nums,
      mfcc5_data.begin() + prob_training_nums + prob_testing_nums);
  testing_label = vector<double>(
      mfcc5_label.begin() + prob_training_nums,
      mfcc5_label.begin() + prob_training_nums + prob_testing_nums);
  ...
  cout << "Accuracy rate on Training is : " << acc_training * 100 << "%"
       << endl;
  cout << "Accuracy rate on Testing is : " << acc_testing * 100 << "%" << endl;

  // 保存模型
  svm_save_model(absolute_saved_path, model);
  cout << endl;
...
int main(int argc, char const* argv[]) {
  training_svm(1,
               "/home/liuzihua/Desktop/media/newest/mediapipe/experimental/"
               "zihualiu/apps/VisemeReco/training/svm_model/svm9p");
  return 0;
}
