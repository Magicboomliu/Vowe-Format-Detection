...
constexpr char kOutputCatagory[] = "output_category";
constexpr char kInputStream[] = "mono_waveform";

mediapipe::Matrix observed_output;
...
// 建立图的输出
graph->ObserveOutputStream(kOutputCatagory,
                             [&](const mediapipe::Packet& category_packet) {
                                observed_output = category_packet.Get<mediapipe::Matrix>();
...
LOG(INFO)<<"这个是输出cols:"<<observed_output.cols()<<"\n";
LOG(INFO)<<"这个是输出rows:"<<observed_output.rows()<<"\n";
}
