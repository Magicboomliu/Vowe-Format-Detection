...
DEFINE_string(calculator_graph_config_file, "",
              "Name of file containing text format CalculatorGraphConfig "
              "proto.");  // Graph的地址
DEFINE_string(input_audio_path, "",
              "Full path of audio to load. ");  // Input_audio的地址

// 定义输入的SidePackage 以及 输入输出流
constexpr char kInputAudioSidePacket[] = "input_audio";
constexpr char kOutputMfcc[] = "viseme";
constexpr char kInputStream[] = "mono_waveform";

mediapipe::Matrix viseme_output;
...
::mediapipe::Status RunMPPGraph() {
  ...
  graph->ObserveOutputStream(
      kOutputMfcc, [&](const mediapipe::Packet& viseme_packet) {
        viseme_output = viseme_packet.Get<mediapipe::Matrix>();
  ...
  while (true) {
    ...
    for (int i = 0; i < viseme_output.rows(); i++) {
      for (int j = 0; j < viseme_output.cols(); j++) {
        if (j == viseme_output.cols() - 1) {
          std::cout << viseme_output(i, j) << " \n ";
        } else {
          std::cout << viseme_output(i, j) << " , ";
        }
      }
